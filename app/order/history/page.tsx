            ...
            <div className={styles.stars}>
                {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} type="button"
                            ...
                            onClick={() => onChange(star)} aria-label={`${star} bintang`}>★</button>
                ))}
            </div>
...
function RatingModal({ orderId, onClose, onSuccess }: { orderId: string; onClose: () => void; onSuccess: () => void }) {
    ...
    const handleSubmit = async () => {
        if (!jastiperRating || !productRating) { setError("Mohon isi semua rating sebelum mengirim."); return; }
        setLoading(true); setError("");
        try {
            const res = await fetch(
                `${API}/api/v1/orders/${orderId}/rate?jastiperRating=${jastiperRating}&productRating=${productRating}`,
                { method: "POST" }
            );
            if (res.ok) { onSuccess(); onClose(); }
            else { const msg = await res.text(); setError(msg || "Gagal mengirim rating."); }
        ...
        <div className={styles.backdrop} onClick={onClose}>
            <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
                ...
                <div className={styles.modalActions}>
                    <button className={styles.btnSecondary} onClick={onClose} disabled={loading}>Batal</button>
...
export default function OrderHistoryPage() {
    ...
    const fetchOrders = async () => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`${API}/api/v1/orders/history/${user.id}`);
            if (res.ok) {
    ...
    const handleCancel = async (orderId: string) => {
        ...
        try {
            ...
            if (res.ok) { showToast("Pesanan dibatalkan. Saldo sudah di-refund!", "success"); fetchOrders(); }
    ...
    const handleConfirmReceived = async (orderId: string) => {
        ...
        try {
            const res = await fetch(`${API}/api/v1/orders/${orderId}/completed`, { method: "PATCH" });
            if (res.ok) { showToast("Pesanan selesai! Jangan lupa beri penilaian ya 😊", "success"); fetchOrders(); }
            else { const msg = await res.text(); showToast(msg || "Gagal mengkonfirmasi penerimaan.", "error"); }
        } catch { showToast("Tidak dapat terhubung ke server.", "error"); }
    };
    ...
    if (loading) return (
        <div className={styles.loadingWrapper}><div className={styles.spinner} /><p>Memuat pesanan...</p></div>
    );
        ...
        <div className={styles.page}>
                ...
                {filtered.length === 0 ? (
                    <div className={styles.emptyState}>
                        ...
                        <p className={styles.emptyDesc}>
                            {activeTab === "SEMUA" ? "Yuk hunting barang jastip pertamamu!" : `Tidak ada pesanan dengan status ${activeTab}.`}
                        </p>
                    </div>
                ) : (
                    filtered.map((order) => (
                        <div key={order.id} className={styles.card}>
                            ...
                            <div className={styles.details}>
                                <p className={styles.productName}>{order.productName || order.productId}</p>
                                <div className={styles.detailGrid}>
                                    <div className={styles.detailItem}>
                                        <span className={styles.detailLabel}>Total Harga</span>
                                        <span className={styles.detailValue}>Rp {order.totalPrice.toLocaleString("id-ID")}</span>
                                    </div>
                                    <div className={styles.detailItem}>
                                    ...
                                    {order.trackingNumber && (
                                        <div className={styles.detailItem}>
                                            ...
                                            <span className={`${styles.detailValue} ${styles.resi}`}>📦 {order.trackingNumber}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            <div className={styles.actionGroup}>
                                {order.status === "PAID" && (
                                    <button className={styles.btnCancel} onClick={() => handleCancel(order.id)}>Batalkan Pesanan</button>
                                )}
                                {order.status === "SHIPPED" && (
                                    <button className={styles.btnConfirm} onClick={() => handleConfirmReceived(order.id)}>✓ Konfirmasi Terima</button>
                                )}
                                {order.status === "COMPLETED" && !order.jastiperRating && (
                                    <button className={styles.btnRate} onClick={() => setRatingOrderId(order.id)}>⭐ Beri Penilaian</button>
                                )}
                                {order.status === "COMPLETED" && order.jastiperRating && (
                                    <div className={styles.ratingDisplay}>
                                        <span>🧳 Jastiper: {"★".repeat(order.jastiperRating)}{"☆".repeat(5 - order.jastiperRating)}</span>
                                        <span>📦 Produk: {"★".repeat(order.productRating ?? 0)}{"☆".repeat(5 - (order.productRating ?? 0))}</span>
                                    </div>
                                )}
                            </div>
                        </div>
            ...
            {ratingOrderId && (
                <RatingModal orderId={ratingOrderId} onClose={() => setRatingOrderId(null)}
                             onSuccess={() => { showToast("Terima kasih atas penilaianmu! 🎉", "success"); fetchOrders(); }} />
            )}
        </div>
    );
}